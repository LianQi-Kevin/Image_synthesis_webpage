        ...
        with gr.Column():
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    output_img = gr.Gallery(interactive=False, show_label=False).style(grid=[3], height="640px")
        # style
    ...
    with gr.Blocks(title="109美术高中AI与美术融合课", css="utils/gradio_css.css") as advanced_app:
        with gr.Column():
            ...
            with gr.Row():
                ...
                with gr.Column():
                    gr.Markdown("### Setting")
                    seed_box = gr.Number(value=np.random.randint(1, 100000), label="Seed",
                                         interactive=True, precision=0)
                    with gr.Row():
                        grid_size_slider = gr.Slider(value=2, minimum=1, maximum=5, step=1, label="Grid Size")
                        random_seed_checkbox = gr.Checkbox(value=False, label="Random Seed")
                    ...
                    with gr.Row():
                        ...
                        supercondition_dropdown = gr.Dropdown(label="Super Condition", value="16",
                                                              ...
                                                              interactive=True)
                    grid_path_textbox = gr.Textbox(interactive=False, visible=True, label="Grid_img output path")
        ...
        # action
        random_seed_checkbox.change(random_seed, inputs=[random_seed_checkbox], outputs=[seed_box])
        prompt_box.submit(run_model,
                          ...
                          outputs=[output_gallery, grid_path_textbox])
