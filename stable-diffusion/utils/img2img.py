...
class img2img(object):
    def __init__(self, ckpt, config, n_rows=2, output_dir="outputs/img2img-samples", precision="autocast"):
        # load model
        self.ckpt = ckpt
        self.config = config
    ...
    def _load_init_img(self, img_path: str, batch_size: int):
        ...
        init_image = load_image(img_path).to(self.device)
        init_image = repeat(init_image, '1 ... -> b ...', b=batch_size)
        init_latent = self.model.get_first_stage_encoding(
            self.model.encode_first_stage(init_image))  # move to latent space
        return init_latent

    def synthesis(self, prompt, init_img, seed=42, n_samples=4, n_iter=4,
