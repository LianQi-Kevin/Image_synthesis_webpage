...
def postprocess(all_samples, n_rows):
...
def main():
    ...
    # ----------
    # 调试用 覆盖args
    opt.prompt = "sunset, sunrays showing through the woods in front, clear night sky, stars visible, mountain in the back, lake in front reflecting the night sky and mountain, photo realistic, 8K, ultra high definition, cinematic"
    ...
    opt.ddim_steps = 50
    opt.skip_save = True
    opt.seed = np.random.randint(1, 100)    # seed相同时结果相同
    # ----------
    ...
    if not opt.skip_grid:
        postprocess(all_samples, n_rows).save(os.path.join(out_path, f'grid-{grid_count:04}.png'))
